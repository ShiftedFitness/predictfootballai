...
// normalize + lightweight fuzzy
function normalize(s){ return String(s||'').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g,'').replace(/[^a-z0-9\s.-]/g,'').trim(); }
function lev(a,b){
...
async function fetchAll(){
  ...
  while(true){
    const url = `${ADALO_API_BASE}/collections/${PREMIER_LEAGUE_PLAYERS_COLLECTION}?offset=${offset}&limit=1000`;
    const res = await fetch(url,{ headers:{ Authorization:`Bearer ${ADALO_API_KEY}`,'Content-Type':'application/json' }});
    if(!res.ok){ throw new Error(`Adalo ${res.status}`); }
    const data = await res.json();
    const rows = (data.records||[]).filter(r => r && r.Player);
    for(const r of rows){
      all.push({ player: r.Player, matches: Number(r.Matches||0), norm: normalize(r.Player) });
    }
...
exports.handler = async (event) => {
  try{
    ...
    // similarity fallback
    if(!best){
      let top = { item:null, conf:0, rec:null };
      for(const it of list){
        const s = sim(it.norm, target);
        if(s > top.conf) top = { item: it.player, conf: s, rec: it };
      }
